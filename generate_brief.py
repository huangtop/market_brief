...
在寫入報告前，必須先判斷事件在「現在台北時間 {now.strftime('%Y/%m/%d %H:%M')}」屬於：
...
────────────────────
6. Citation entailment｜來源必須真的支持句子
────────────────────

最終輸出前，對每一則事件做一次來源對齊檢查。

來源必須直接支持該事件標題與核心事實中的：
...
來源必須直接是：
- Cisco 8/12 該次財報官方頁；或
- 直接報導 Cisco 8/12 該次財報的可靠新聞。
...
來源不能只有：
「AMAT Q3 conference call event 預告頁」。

必須有實際 Q3 results、新聞稿或直接報導該次結果的來源。

如果來源無法直接支持句子：

1. 找得到正確來源且仍在查證預算內 → 換來源。
2. 找不到 → 刪掉不被支持的細節。
3. 核心事件本身無法支持 → 整則刪除。

禁止因為來源來自公司官網就假設它一定支持目前句子。
「官方來源但日期／季度不對」仍視為錯誤來源。
...
- PPI 低於預期 → 美債殖利率下降 → 長久期科技股估值壓力減輕
- AMAT 上修 WFE 展望 → 半導體設備族群可能重新定價
- Cisco AI orders 增長 → AI networking / switch 供應鏈 read-through
...
<article class="mb-brief">
  ...
  <section class="mb-section mb-catalysts">
    <h3>未來48小時催化劑</h3>
    <ul>
      <li>只列目前時間之後、尚未發生且真正值得注意的具體催化劑。</li>
    </ul>
  </section>
...
- 是否先用最多一次檢視找昨夜科技異常族群。
- 若 Finviz 無法使用，是否最多只做一次 fallback。
- 是否只建立輕量近期財報候選池。
- 是否完成「異常族群 × 財報／催化劑」交叉。
- 是否只做一次廣泛的今日新增重大事件搜尋。
...
- 每個有時間的事件，是否已判定為「尚未發生／正在發生／已發生」。
- 已發生的財報／法說，是否使用實際結果而非 event 預告頁。
- 已經發生的事件是否沒有被錯放進「未來48小時催化劑」。
- 今日觀察是否反映目前事件狀態，而不是還在等待已經公布的內容。

來源檢查：

- 每個來源是否直接支持該句的公司、日期、動作與季度。
- 是否存在「來源是真的，但其實是舊季度／舊日期」。
- ★★★★☆／★★★★★ 是否有具體事件頁面，而不是 topic / category 首頁。
- 若來源不支持細節，是否已刪除該細節或整則事件。
